...
namespace MSBuildCustomTasks
{
    public class DeActivateProjectJson : CallTarget
    {
        public override bool Execute()
        {
            ...
            {
                Rename(projectJson, projectJsonDeactivate);
            }
            else if (File.Exists(projectJsonDeactivate))
            {
                Log.LogMessage(MessageImportance.Normal,"project.json has allready been deactivated.");
            }
            else
            {
                Log.LogError("Unable to deactivate project.json. There is no project.json to be found in project folder '{0}'.", ProjectFolder);
                return false;
            }
            return base.Execute();
        }
